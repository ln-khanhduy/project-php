...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$email || $password === '') {
        ...
        $stmt = $db->prepare('SELECT user_id, full_name, email, password, role, provider, oauth_id, is_locked FROM users WHERE email = :email LIMIT 1');
        ...
        if (!$user) {
            ...
            $alertMessage = 'Tài khoản đang bị khóa.';
        } else {
